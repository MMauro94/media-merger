...
data class CachedBlackSegments(private val map: MutableMap<FFMpegBlackdetectThresholds, CachedBlackSegmentsForConfig> = mutableMapOf()) {

    operator fun get(config: FFMpegBlackdetectThresholds) = map.getOrPut(config, { CachedBlackSegmentsForConfig() })

    operator fun set(config: FFMpegBlackdetectThresholds, range: DurationSpan?, blackSegments: List<DurationSpan>) {
        ...
        fun fromJsonArray(arr: JsonArray<*>): CachedBlackSegments {
            return CachedBlackSegments(arr.associate {
                if (it is JsonObject) {
                    ...
                    val config = KLAXON.parseFromJsonObject<FFMpegBlackdetectThresholds>(jConfig) ?: throw KlaxonException("Invalid config")
